...
cfg = {
    'name': 'Resnet50',
    'min_sizes': [[16, 32], [64, 128], [256, 512]], # return to 5-12
    'steps': [8, 16, 32],
    'variance': [0.1, 0.2],
    'clip': False,
    'pretrain': True,
...
def face_align_crop(net, img_paths, device):
    ...
    for img_path in img_paths:
        # check if path leads to an image
        if img_path.split('.')[-1] not in ['JPG', 'PNG', 'GIF', 'jpg', 'png', 'gif', 'jpeg']:
            continue

        if args.use_subdirectories:
            subdir_name = os.path.split(os.path.split(img_path)[-2])[-1]
            save_dir = os.path.join(args.save_dir, subdir_name)
...
def detect(net, img_path, img_name, device, save_dir='', first=False):
    ...
    def resize_square_aspect_cv2(img, desired_size=640):
        ...
        if desired_size == 0:
            ...
            if (old_size[0] * old_size[1]) > 1000000:
                desired_size = 1024  # should be large enough
    ...
    try:
        ...
        print(f"Error at image: {img_path}")
        return None, []
...
def align(img, coords, img_name, save_img=True, save_dir='', verbose=True):
    ...
    if save_img:
        os.makedirs(save_dir, exist_ok=True)
        if verbose:
            save_loc = os.path.join(save_dir, f'{img_name}_aligned.jpg')
