...
class NumpyCompatibleUnpickler(pickle.Unpickler):
    """
    Allows pickle files created with NumPy 2.x (numpy._core)
    to be read in a NumPy 1.x environment (numpy.core).
    """
...
def read_encoding_pickle_compatible(input_file):
    try:
        return pd.read_pickle(input_file)
    except ModuleNotFoundError as error:
        if "numpy._core" not in str(error):
            raise
        print("Detected a NumPy 2.x pickle; applying NumPy 1.x compatibility loader.")
        with open(input_file, "rb") as file:
            return NumpyCompatibleUnpickler(file).load()
...
def main():
    ...
    encoding_df = read_encoding_pickle_compatible(args.case_input)
